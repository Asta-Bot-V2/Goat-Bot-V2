...
async function convertImageToText(imageURL) {
  try {
    const response = await axios.get(`https://mota-dev.x10.bz/ai?prompt=&name=${encodeURIComponent(imageURL)}`);
...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  try {
    ...
    const response = await axios.get(`https://mota-dev.x10.bz/ai?prompt=&name=${encodeURIComponent(question)}`);
